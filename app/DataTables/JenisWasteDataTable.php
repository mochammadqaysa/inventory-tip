...
class JenisWasteDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($item) {
                ...
                if (in_array($user->role->slug, ['super_admin'])) {
     ...
     *
     * @param \App\Models\JenisWaste $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(JenisWaste $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        ...
        $user = AuthCommon::getUser();
        if (in_array($user->role->slug, ['super_admin'])) {
            $button[] = Button::raw('<i class="fa fa-plus"></i> Create Jenis Waste')->action('function() { create() }');
        }
        return $this->builder()
            ->parameters([
                'language' => [
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('kode'),
            Column::make('nama')->title("Nama Jenis Waste"),
        ];
    }
    ...
    {
        return 'JenisWaste_' . date('YmdHis');
    }
}
