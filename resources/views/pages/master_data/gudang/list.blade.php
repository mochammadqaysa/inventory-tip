...
<div class="col-lg-6 col-7">
  ...
  <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
    <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
      <li class="breadcrumb-item"><a href="#"><i class="fas fa-warehouse"></i></a></li>
...
<script>
  let _url = {
    create: `{{ route('gudang.create') }}`,
    edit: `{{ route('gudang.edit', ':id') }}`,
    show: `{{ route('gudang.show', ':id') }}`,
    destroy: `{{ route('gudang.destroy', ':id') }}`
  }
  ...
  function save(){
    ...
    $.ajax({
      ...
      if(res?.status == true){
        ...
        window.LaravelDataTables["gudang-table"].draw()
      }
    }).fail((xhr) => {
      if(xhr?.status == 422){
