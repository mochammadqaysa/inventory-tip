...
<div class="col-lg-6 col-7">
  ...
  <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
    <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
      <li class="breadcrumb-item"><a href="#"><i class="fas fa-cube"></i></a></li>
...
<script>
  let _url = {
    create: `{{ route('bahan.create') }}`,
    edit: `{{ route('bahan.edit', ':id') }}`,
    show: `{{ route('bahan.show', ':id') }}`,
    destroy: `{{ route('bahan.destroy', ':id') }}`
  }
  ...
  function save(){
    ...
    $.ajax({
      ...
      if(res?.status == true){
        ...
        window.LaravelDataTables["bahan-table"].draw()
      }
    }).fail((xhr) => {
      if(xhr?.status == 422){
