...
<div class="col-lg-6 col-7">
  ...
  <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
    <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
      <li class="breadcrumb-item"><a href="#"><i class="fas fa-dumpster"></i></a></li>
      <li class="breadcrumb-item active" aria-current="page">Waste</li>
    </ol>
  </nav>
</div>
@endsection
...
<script>
  let _url = {
    create: `{{ route('waste.create') }}`,
    edit: `{{ route('waste.edit', ':id') }}`,
    show: `{{ route('waste.show', ':id') }}`,
    destroy: `{{ route('waste.destroy', ':id') }}`
  }
  ...
  function save(){
    ...
    $.ajax({
      ...
      if(res?.status == true){
        ...
        window.LaravelDataTables["waste-table"].draw()
      }
    }).fail((xhr) => {
      if(xhr?.status == 422){
