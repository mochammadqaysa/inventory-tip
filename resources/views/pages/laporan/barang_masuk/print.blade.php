...
<body>
  ...
  <table id="table">
      ...
      <tbody>
        ...
        <tr>
          @if($item->barangMasuk->nomor_bukti != $lastNomorBukti)
            <td>{{ $iteration }}</td>
            <td style="text-align: left">{{ $item->barangMasuk->nomor_bukti }} </td>
          ...
          @else
              <td></td>
          @endif --}}
          <td>{{ Utils::formatTanggalLaporan($item->barangMasuk->tanggal_bukti) }}</td>
          ...
          <td>{{ $item->barangMasuk->gudang->nama }}</td>
        </tr>
        @php
            $lastNomorBukti = $item->barangMasuk->nomor_bukti;
        @endphp
        @endforeach
        <tr class="xls-ignore">
...
<script>
  function tableToExcel(content, filename) {
      $.ajax({
          url: "{{ route('excel-report.barang-masuk') }}",
          headers: {
  ...
  function handleExport() {
    ...
    const filename = `LAPORAN-PEMASUKAN-BARANG-JADI-${yyyy}${mm}${dd}${h}${i}${s}`; // Add your desired filename here
