...
@section('title', 'Pemasukan Barang Jadi')
...
<div class="col-lg-6 col-7">
  ...
  <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
    <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
      <li class="breadcrumb-item"><a href="#"><object type="image/svg+xml" data="{{asset('assets/img/brand/import_barang.svg')}}" class="custom-icon custom-icon-small custom-icon-white"></object></a></li>
      <li class="breadcrumb-item active" aria-current="page">Pemasukan Barang Jadi</li>
    </ol>
  </nav>
</div>
@endsection
...
<script>
  let _url = {
    create: `{{ route('barang-masuk.create') }}`,
    edit: `{{ route('barang-masuk.edit', ':id') }}`,
    show: `{{ route('barang-masuk.show', ':id') }}`,
    destroy: `{{ route('barang-masuk.destroy', ':id') }}`
  }
  ...
  function save(){
    ...
    $.ajax({
      ...
      if(res?.status == true){
        ...
        window.LaravelDataTables["barangmasuk-table"].draw()
        Ryuna.close_modal()
      }
    }).fail((xhr) => {
      if(xhr?.status == 422){
