...
@section('title', 'Pengeluaran Waste / Scrap')
...
<script>
  let _url = {
    create: `{{ route('waste-keluar.create') }}`,
    edit: `{{ route('waste-keluar.edit', ':id') }}`,
    show: `{{ route('waste-keluar.show', ':id') }}`,
    destroy: `{{ route('waste-keluar.destroy', ':id') }}`
  }
  ...
  function save(){
    ...
    $.ajax({
      ...
      if(res?.status == true){
        ...
        window.LaravelDataTables["wastekeluar-table"].draw()
        Ryuna.close_modal()
      }
    }).fail((xhr) => {
      if(xhr?.status == 422){
